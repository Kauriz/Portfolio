<?php
    ...
    $pageTemplate = "accueilE-Chat.tpl";
    ...
    // On va chercher les différentes associations tag taches dans la base de donnée

    $stmt = $mysqli->prepare("SELECT * FROM Tag_Tache");
    $stmt->execute();
    $result = $stmt->get_result();
    $tagTaches = $result->fetch_all(MYSQLI_ASSOC);
    $stmt->close();

    //On envoie à Smarty
    $smarty->assign("tags", $tags);
    $smarty->assign("taches", $taches);
    $smarty->assign("tagTaches", $tagTaches);
?>