...
useTexture.preload([
  BASE + 'textures/wall/wall_diff_1k.png',
  BASE + 'textures/wall/wall_nor_gl_1k.png',
  BASE + 'textures/wall/wall_rough_1k.png',
  BASE + 'textures/floor/floor_02_diff_1k.png',
  BASE + 'textures/floor/floor_02_nor_gl_1k.png',
  BASE + 'textures/floor/floor_02_rough_1k.png',
  BASE + 'textures/baseboard/baseboard_diff_1k.png',
  BASE + 'textures/baseboard/baseboard_nor_gl_1k.png',
  BASE + 'textures/baseboard/baseboard_rough_1k.png',
])

useGLTF.preload(BASE + 'models/frame/frame.gltf')
useGLTF.preload(BASE + 'models/sofa/sofa.gltf')
useGLTF.preload(BASE + 'models/door.glb')
useGLTF.preload(BASE + 'models/dresser.glb')
useGLTF.preload(BASE + 'models/lamp.glb')
useGLTF.preload(BASE + 'models/openBook.glb')
useGLTF.preload(BASE + 'models/wallLamp.glb')
useGLTF.preload(BASE + 'models/pedestal.glb')
useGLTF.preload(BASE + 'models/concrete_cat_statue_1k.gltf/concrete_cat_statue_1k.gltf')
useGLTF.preload(BASE + 'models/ropeBarrier.glb')
useGLTF.preload(BASE + 'models/linkedin.glb')
useGLTF.preload(BASE + 'models/mail.glb')
useGLTF.preload(BASE + 'models/piano.glb')
useGLTF.preload(BASE + 'models/bookshelf.glb')
useGLTF.preload(BASE + 'models/sleepingCat.glb')
useGLTF.preload(BASE + 'models/table.glb')
useGLTF.preload(BASE + 'models/chair.glb')
useGLTF.preload(BASE + 'models/chess.glb')
useGLTF.preload(BASE + 'models/paintingLamp.glb')
useGLTF.preload(BASE + 'models/lightSwitch.glb')
...
export function Door({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/door.glb')
  const clone = useMemo(() => {
    const c = scene.clone()
...
export function Sofa({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/sofa/sofa.gltf')
  const clone = useMemo(() => {
    const c = scene.clone()
    c.traverse((child) => {
      if (child.isMesh) {
...
export function Dresser({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/dresser.glb')
  const clone = useMemo(() => {
    const c = scene.clone()
    c.traverse((child) => {
      if (child.isMesh) {
...
export function OpenBook({ position, scale, rotation }) {
  const { scene } = useGLTF(BASE + 'models/openBook.glb')
  const clone = useMemo(() => {
    const c = scene.clone()
    const box = new THREE.Box3().setFromObject(c)
    const center = box.getCenter(new THREE.Vector3())
...
export function WallLamp({ position, scale, rotation }) {
  const { scene } = useGLTF(BASE + 'models/wallLamp.glb')
  const clone = useMemo(() => {
    const c = scene.clone()
    const box = new THREE.Box3().setFromObject(c)
    const center = box.getCenter(new THREE.Vector3())
...
export function CatStatue({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/concrete_cat_statue_1k.gltf/concrete_cat_statue_1k.gltf')
  const clone = useMemo(() => scene.clone(), [scene])
  return (
    <primitive
      object={clone}
...
export function RopeBarrier({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/ropeBarrier.glb')
...
export function Linkedin({ position, scale, rotation }) {
  const { scene } = useGLTF(BASE + 'models/linkedin.glb')
  const clone = useMemo(() => {
    const c = scene.clone()
    const box = new THREE.Box3().setFromObject(c)
    const center = box.getCenter(new THREE.Vector3())
...
export function Mail({ position, scale, rotation }) {
  const { scene } = useGLTF(BASE + 'models/mail.glb')
  const clone = useMemo(() => {
    const c = scene.clone()
    const box = new THREE.Box3().setFromObject(c)
    const center = box.getCenter(new THREE.Vector3())
...
export function Piano({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/piano.glb')
...
export function Bookshelf({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/bookshelf.glb')
...
export function SleepingCat({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/sleepingCat.glb')
...
export function Table({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/table.glb')
...
export function Chair({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/chair.glb')
...
export function Chess({ position, scale, rotation }) {
  const { scene } = useGLTF(BASE + 'models/chess.glb')
  const clone = useMemo(() => {
    const c = scene.clone()
    const box = new THREE.Box3().setFromObject(c)
    const center = box.getCenter(new THREE.Vector3())
...
export function PaintingLamp({ position, scale, rotation, playerPositionRef }) {
  const { scene } = useGLTF(BASE + 'models/paintingLamp.glb')
...
export function LightSwitch({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/lightSwitch.glb')
...
export function Plant1({ position, scale, rotation }) {
  const { scene } = useGLTF(BASE + 'models/plant1.glb')
  const clone = useMemo(() => {
    const c = scene.clone()
    const box = new THREE.Box3().setFromObject(c)
    const center = box.getCenter(new THREE.Vector3())
...
export function Desk({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/desk.glb')
...
export function ChalkBoard({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/chalkboard.glb')
...
export function Easel({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/easel.glb')
...
export function Canva({ position, scale, rotation }) {
  const { scene } = useGLTF(BASE + 'models/canvas.glb')
  const clone = useMemo(() => {
    const c = scene.clone()
    const box = new THREE.Box3().setFromObject(c)
    const center = box.getCenter(new THREE.Vector3())
...
export function UnderShelf({ position, rotation, scale }) {
  const { scene } = useGLTF(BASE + 'models/underShelf.glb')
